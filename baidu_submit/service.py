...
from baidu_submit.job import UrlSubmitJob
...
_COOKIE_FILE_INVALID = app_env.get_app_root() + "/baidu_submit/cookie-invalid.txt"
_COOKIE_EXPIRE_COUNT = 10
_THREAD_SIZE = 2
_PROXY_CONF = json.load(open(app_env.get_app_root() + "/baidu_submit/proxy.json", encoding="UTF-8"))
_PROXY_REPO = "http://www.xdaili.cn/ipagent/greatRecharge/getGreatIp?spiderId={}&orderno={}&returnType=2&count=1"
...
class BaiduSubmit:
    ...
    def append_buffer(self, url, priority):
        self._url_buffer.put(UrlSubmitJob(url, priority))
